...
int mm_init(void)
{
    ...
    heap_listp += (2 * WSIZE);                     // move pointer to prologue header (???)
...
static void place(size_t *bp, size_t asize)
{
    ...
    if (0 < nsize) // Free remaining block
    {
...
{
    /* First-Fit */
    // void *bp = mem_heap_lo() + 4 * WSIZE; // First block pointer in free list, it will be next bp.
    ...
    /* Next-Fit */
    void *bp_start = mem_heap_lo() + 4 * WSIZE; // First block pointer in free list

    if (bp_before == NULL) // Initialization
        bp_before = bp_start;
    ...
    {
        if (GET_ALLOC(HDRP(bp)) == 0 && asize <= GET_SIZE(HDRP(bp))) // If block(bp) is free and fitting, return bp.
        {
            bp_before = bp;
            return bp;
        }

        if (GET(HDRP(NEXT_BLKP(bp))) == 0x1) // Checking next block is in heap(if word in bp block header is 0x1, it means epilogue header.)
            bp = bp_start;
        else // If next block isn't(header is epilogue block), traverse from initial block in heap
            bp = NEXT_BLKP(bp);
...
void *mm_realloc(void *oldptr, size_t newsize)
{
    ...
    /* When unable to find free space at block itself or next block, just use mm_malloc and mm_free function */
